...
/*
----------------------------------------------------------------------------------------------------
    Essa tela lista os alimentos que o usuário ingeriu no café da manhã
    Filtrar por "DebugMax" no logcat para ver as mensagens de debug do programador
----------------------------------------------------------------------------------------------------*/

public class List_Cafe extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Chama a próxima tela com a referencia de qual refeição é essa (id refeição da tabela de refeicao, não o tipo, por isso a consulta no banco)
...
----------------------------------------------------------------------------------------------------*/
    ...
    public void carregarDados(){
        ...
        try {
            ...
            Cursor cursor_alimentos = bancoDados.rawQuery("SELECT Alimento_Refeicao.id, Alimento_Refeicao.qntd, alimentos.nome" +
                    ...
                    " WHERE refeicoes.data = current_date AND refeicoes.tipo = '1'",null); // Trocar conforme refeição
    ...
    public void consultaId() {
        bancoDados = openOrCreateDatabase("bancoUserData", MODE_PRIVATE, null);
        Cursor cursor = bancoDados.rawQuery("SELECT id FROM refeicoes WHERE data = current_date AND tipo = '1'",null); //trocar o tipo da refeição conforme a tela
        cursor.moveToFirst();
    ...
    public void criarRefeicaoDeHoje () {
        bancoDados = openOrCreateDatabase("bancoUserData", MODE_PRIVATE, null);
        Cursor cursor = bancoDados.rawQuery("SELECT * FROM refeicoes WHERE data = current_date AND tipo = '1'",null); //trocar o tipo da refeição conforme a tela
        if(!cursor.moveToFirst()) {

            String sql = "INSERT INTO refeicoes(tipo,data) values ('1',current_date)";  //trocar o tipo da refeição conforme a tela
