...
/*
----------------------------------------------------------------------------------------------------
    Essa tela lista os alimentos que o usuário ingeriu na janta
    Filtrar por "DebugMax" no logcat para ver as mensagens de debug do programador
----------------------------------------------------------------------------------------------------*/

public class List_Janta extends AppCompatActivity {
...
----------------------------------------------------------------------------------------------------*/
    ...
    public void carregarDados(){
        ...
        try {
            ...
            Cursor cursor_alimentos = bancoDados.rawQuery("SELECT Alimento_Refeicao.id, Alimento_Refeicao.qntd, alimentos.nome" +
                    ...
                    " WHERE refeicoes.data = current_date AND refeicoes.tipo = '4'",null); // Trocar conforme refeição
    ...
    public void consultaId() {
        bancoDados = openOrCreateDatabase("bancoUserData", MODE_PRIVATE, null);
        Cursor cursor = bancoDados.rawQuery("SELECT id FROM refeicoes WHERE data = current_date AND tipo = '4'",null); //trocar o tipo da refeição conforme a tela
        cursor.moveToFirst();
    ...
    public void criarRefeicaoDeHoje () {
        bancoDados = openOrCreateDatabase("bancoUserData", MODE_PRIVATE, null);
        Cursor cursor = bancoDados.rawQuery("SELECT * FROM refeicoes WHERE data = current_date AND tipo = '4'",null); //trocar o tipo da refeição conforme a tela
        if(!cursor.moveToFirst()) {

            String sql = "INSERT INTO refeicoes(tipo,data) values ('4',current_date)";  //trocar o tipo da refeição conforme a tela
            SQLiteStatement stmt = bancoDados.compileStatement(sql);
            stmt.executeInsert();

            System.out.println("DebugMax: Criando uma nova refeição (Janta) na tabela refeicao no dia de hoje");
        }
        bancoDados.close();
    }
}
